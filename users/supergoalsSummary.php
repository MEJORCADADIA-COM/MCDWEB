...
<script>
   ...
   function loadMoreItems(pageNumber, tag) {
      ...
      const url = `<?= SITE_URL; ?>/users/ajax/ajax.php?get_supergoals_summary=get_supergoals_summary&type=${type}&page=${pageNumber}${tag? `&tag=${tag}`:""}`;
      fetch(url)
         .then(res => res.json())
         .then(data => {
            ...
            dailyVictories.forEach(dailyVictory => {
               if (dailyVictory.description) {
                  ...
                  item.innerHTML =
                     `<div>
                        <p class="my-2">${dailyVictory.description}</p>
                        <div class="d-flex justify-content-between">
                           <p class="text-muted date-font mt-2"> 
                           <small><strong>Date: </strong>${formatDate(new Date(dailyVictory.start_date))} to ${formatDate(new Date(dailyVictory.end_date))}</small>
