...
if (isset($_POST['send_email'])) {
    ...
    if (!empty($mission1)) {
        $missions = "<h4>Misión #1:</h4><div>".html_entity_decode($mission1)."</div>";
    }
    if (!empty($mission2)) {
        $missions .= "<h4>Misión #2:</h4><div>".html_entity_decode($mission2)."</div>";
    }
    if (!empty($mission3)) {
        $missions .= "<h4>Misión #3:</h4><div>".html_entity_decode($mission3)."</div>";
    }
    ...
    try {
        ...
        <ol>
            <div style='width:600px; background-color:#FFF; margin:0 auto;'>
                ...
                <div style='padding:20px; background-color:#FFF;'>
                    <h2 style='text-transform: capitalize;'>Misión o misiones para Mi Vida</h2>
        ...
        if ((new Email())->send('Misión o misiones para Mi Vida', $email, $body)) {
...
<main role="main" class="col-md-9 ml-sm-auto col-lg-10 mb-3 text-white">
    ...
    <div class="projects py-5" style="background-color: #ed008c;">
        <div class="mb-5" style="background-color: #fef200; padding: 10px">
            <h2 class="toptitle text-black " style="padding:0; margin:0; width:100%; overflow:hidden;">Misión o misiones para Mi Vida:</h2>
        </div>
        <form class="px-1 px-lg-0" action="" method="post">
            <div class="mb-4">
                <label class="px-1" style="margin: 5px 0px; font-size:1rem;" for="mission_1">Misión #1:</label>
                <textarea name="mission_1" id="mission_1" class="tinymce-editor form-control w-75 mx-auto form-box shadow-lg border border-light border-opacity-10"><?= $userMissions['mission_1'] ?? '' ?></textarea>
            </div>
            <div class="mb-4">
                <label class="px-1" style="margin: 5px 0px; font-size:1rem;" for="mission_2">Misión #2:</label>
                <textarea name="mission_2" id="mission_2" class="tinymce-editor form-control w-75 mx-auto form-box shadow-lg border border-light border-opacity-10"><?= $userMissions['mission_2'] ?? '' ?></textarea>
            </div>
            <div class="mb-4">
                <label class="px-1" style="margin: 5px 0px; font-size:1rem;" for="mission_3">Misión #3:</label>
                <textarea name="mission_3" id="mission_3" class="tinymce-editor form-control w-75 mx-auto form-box shadow-lg border border-light border-opacity-10"><?= $userMissions['mission_3'] ?? '' ?></textarea>
            </div>
            <button class="btn btn-info letter" type="button" data-bs-toggle="modal" href="#exampleModalToggle" role="button">Email</button>
            <button class="btn btn-info letter" type="submit" name="save_missions">Guardar</button>
                    ...
                    <div class="modal-content">
                        ...
                        <div class="modal-body">
                            <div class="form-group">
                                ...
                                <input style="width:100%;" type="email" class="form-control" name="to_email" id="toEmail" placeHolder="Enter Email Address" required>
                            </div>
                        </div>
