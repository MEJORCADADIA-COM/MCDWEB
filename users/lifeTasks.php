...
if (isset($_POST['send_email'])) {
    ...
    if (!empty($lifeExpectations)) {
        $lifeTasks = "<h4>".translate('¿Qué es lo que Vida Espera de Mí? ¿Qué espera que Haga o no Haga?').":</h4><div>".html_entity_decode($lifeExpectations)."</div>";
    }
    if (!empty($feelHaveToDo)) {
        $lifeTasks .= "<h4>".translate('¿Qué Siento que tengo que Hacer o llevar a Cabo en Mi Vida?').":</h4><div>".html_entity_decode($feelHaveToDo)."</div>";
    }
    if (!empty($thingsToDoBeforeDeath)) {
        $lifeTasks .= "<h4>".translate('¿Qué cosas, tareas o acciones debo de Hacer antes de Morirme?').":</h4><div>".html_entity_decode($thingsToDoBeforeDeath)."</div>";
    }
    if (!empty($bestWayToServeHumanity)) {
        $lifeTasks .= "<h4>".translate('¿Cuál/es es la Mejor Manera de Servir a la Humanidad en el tiempo que me queda?').":</h4><div>".html_entity_decode($bestWayToServeHumanity)."</div>";
    }
    if (!empty($highestUseOfTime)) {
        $lifeTasks .= "<h4>".translate('¿Cuál es el Mejor y el Más Alto uso de Mi Tiempo?').":</h4><div>".html_entity_decode($highestUseOfTime)."</div>";
    }
        ...
        <ol>
            <div style='width:600px; background-color:#FFF; margin:0 auto;'>
                ...
                <div style='padding:20px; background-color:#FFF;'>
                    <h2 style='text-transform: capitalize;'>".translate('Mi tarea de Vida')."</h2>
...
<main role="main" class="col-md-9 ml-sm-auto col-lg-10 mb-3 text-white">
    ...
    <div class="projects py-5" style="background-color: #ed008c;">
        <div class="mb-5" style="background-color: #fef200; padding: 10px">
            <h2 class="toptitle text-black " style="padding:0; margin:0; width:100%; overflow:hidden;"><?=translate('Mi tarea de Vida'); ?>:</h2>
        </div>
        <form class="px-1 px-lg-0" action="" method="post">
            <div class="mb-4">
                <label class="px-1" style="margin: 5px 0px; font-size:1rem;" for="life_expectations"><?=translate('¿Qué es lo que Vida Espera de Mí? ¿Qué espera que Haga o no Haga?'); ?>:</label>
            ...
            <div class="mb-4">
                <label class="px-1" style="margin: 5px 0px; font-size:1rem;" for="feel_have_to_do"><?=translate('¿Qué Siento que tengo que Hacer o llevar a Cabo en Mi Vida?'); ?>:</label>
            ...
            <div class="mb-4">
                <label class="px-1" style="margin: 5px 0px; font-size:1rem;" for="things_to_do_before_death"><?=translate('¿Qué cosas, tareas o acciones debo de Hacer antes de Morirme?'); ?>:</label>
            ...
            <div class="mb-4">
                <label class="px-1" style="margin: 5px 0px; font-size:1rem;" for="best_way_to_serve_humanity"><?=translate('¿Cuál/es es la Mejor Manera de Servir a la Humanidad en el tiempo que me queda?'); ?>:</label>
            ...
            <div class="mb-4">
                <label class="px-1" style="margin: 5px 0px; font-size:1rem;" for="highest_use_of_time"><?=translate('¿Cuál es el Mejor y el Más Alto uso de Mi Tiempo?'); ?>:</label>
            ...
            <button class="btn btn-info letter" type="submit" name="save_life_tasks"><?=translate('Save'); ?></button>
