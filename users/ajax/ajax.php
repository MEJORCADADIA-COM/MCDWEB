...
if ($_SERVER["REQUEST_METHOD"] == "GET" && isset($_GET['get_evolutions']) && $_GET['get_evolutions'] === 'get_evolutions') {
    ...
    if (isset($_GET['tag']) && !empty($_GET['tag'])) {
        ...
        $tags = $common->leftJoinPaginate(
            ...
            ['evolution_user_tag.evolution_id'],
            'evolution_user_tag.evolution_id',
            'desc',
            10,
            'evolution_user_tag.evolution_id'
        ...
        $toRemember = null;
        if ($tags['data']) {
            $toRememberIds = array_column($tags['data'], 'evolution_id');
            $inPlaceHolders = count($toRememberIds) >= 1 ? str_repeat('?,', count($toRememberIds) - 1) . '?' : '?';
            $evolutions = $common->get(table: 'dailygaols', cond: 'id IN (' . $inPlaceHolders . ')', params: $toRememberIds, orderBy: 'created_at', order: 'desc');
        }
    } else {
        if (!empty($selectedDate)) {
