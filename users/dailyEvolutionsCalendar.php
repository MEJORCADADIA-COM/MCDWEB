...
if (isset($_POST['update_daily_victory'])) {
    ...
    if (empty($dailyVictory)) {
        Session::set('error', 'Daily Evolution can not be empty.');
        header("Location: $currentUrl");
        return;
    }
...
$monthlyDailyEvolutions = getMonthlyDailyToEvolutionWithTags($user_id, $month, $year);


$dates = array_column($monthlyDailyEvolutions, 'created_at');
$monthlyDailyEvolutions = array_combine($dates, $monthlyDailyEvolutions);
//print_r($monthlyDailyEvolutions);
$monthlyEvolutions = $common->first('monthly_evolutions', 'user_id = :user_id AND month_year = :month_year', ['user_id' => $user_id, 'month_year' => "{$month}_{$year}"]);


function getTDClass($day, $monthlyDailyEvolutions): string
{
    $class = 'border day-box ';
    $class .= !empty($_GET['date']) && $day === (int)$_GET['date'] ? 'target-date ' : '';
    $class .= $monthlyDailyEvolutions ? 'pointer' : '';
...
<main role="main" class="col-md-9 ml-sm-auto col-lg-10 text-white">
    <div class="projects min-vh-100 px-0 px-lg-3 pb-3" style="background-color: #ed008c;">
        ...
        <div class="d-flex justify-content-between px-1 px-lg-0">
            ...
            <a class="text-white" href="<?= SITE_URL . "/users/dailyEvolutionsCalendar.php?month_year={$nextMonthYear}"; ?>"><i class="fa fa-arrow-right fs-4"></i></a>
        </div>
        <br>
                ...
                while ($day <= $numOfDays) {
                    ...
                    for ($j = 1; $j <= 7; $j++) {
                        if ($day === 1 && $j < $firstDay) {
                            ...
                            if ($day <= $numOfDays) {
                                $monthlyDailyVictory = $monthlyDailyEvolutions[date('Y-m-d', strtotime("{$year}-{$month}-{$day}"))] ?? null;
                                
                                $bgColor=empty($monthlyDailyVictory['color'])? '#ffffff':trim($monthlyDailyVictory['color']);
                                echo "<td 
                                style='height: 80px; overflow:hidden; background-color:".$bgColor."' class='" . getTDClass($day, $monthlyEvolutions) . "' " . ($monthlyDailyVictory ? "onClick='openModal(modal{$monthlyDailyVictory['id']}, event)'" : "") . ">
...
<script>
    document.addEventListener('alpine:init', () => {
        Alpine.data('monthlyEvolutions', () => ({
            ...
            async savemonthlyEvolutions() {
                let monthlyEvolutions = window.editors['monthlyEvolutions'].getData().trim();
                if (monthlyEvolutions === '') {
            ...
            formData.append('action', 'UpdateMonthlyEvlolutionNote');
            formData.append('year', this.year);
            formData.append('month', this.month);
            formData.append('evolution', monthlyEvolutions);
                const request = await fetch(`<?= SITE_URL ?>/users/ajax/ajax.php`, {
                    method: 'POST',
                    body:formData
