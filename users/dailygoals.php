...
.custom-audio-picker button:hover,
.custom-audio-picker button.btn.show,
.custom-audio-picker button.btn:active {
    background: #e2e2e2;
}
...
.custom-audio-picker .dropdown-menu .inputfile+label,
.custom-audio-picker .dropdown-menu .dropdown-item {
    border: none;
    width: 100%;
    color: #000;
    ...
    <div class="projects my-5" style="background-color: #ed008c;">
        <div class="projects-inner">
            <header class="projects-header">
                    ...
                    <div class="col-sm-3">
                        <div class="input-group date daily-datepicker datepicker" id="datepicker">
                            <?php $df = 'd-m-Y';
                                ...
                                ?>
                            <input type="text" class="form-control" value="<?= date($df, strtotime($currentDate)); ?>"
                                id="date" readonly />
                ...
                ?>
                <div class="row">
                    <div class="col-sm-2 col-2" style="text-align:left;"><a class="prev-arrow"
                            href="<?= SITE_URL; ?>/users/dailygoals.php?date=<?= date('Y-m-d', strtotime('-1 day', strtotime($currentDate))); ?>"
                            ;><i class="fa fa-arrow-left"></i></a></div>
                    <div class="col-sm-8 col-8" style="text-align:center;">
                        <h2 style="text-transform: capitalize;">
                            <?= utf8_encode(strftime("%A, %d %B, %Y", $dateObj->getTimestamp())); ?></h2>
                    </div>
                    <div class="col-sm-2 col-2" style="text-align:right;"><a class="next-arrow"
                            href="<?= SITE_URL; ?>/users/dailygoals.php?date=<?= date('Y-m-d', strtotime('+1 day', strtotime($currentDate))); ?>"><i
            ...
            <div>
                <button class="btn btn-primary rounded-circle fixed-save-btn text-white" type="button"
                    id="floatingSaveBtn" name="saveBtn"><i class="fa fa-save fa-lg"></i></button>
            </div>
            ...
            <div class="d-flex w-100 slider">
                <div class="pt-5" id="slide-1">
                    <form class="form" id="goalsFrom">
                        <div class="mt-5" style="background-color: #fef200; padding: 10px">
                            <h2 class="maintitle" style="padding:0; margin:0; width:100%; overflow:hidden; ">
                                <?= translate('7-Objetivos y Prioridades Hoy'); ?>:
                                <?php if ($isPastDate == false) : ?>
                                <button type="button" class="btn btn-info btn-sm screenonly pull-right"
                                    id="editBtn1"><?= translate('Editar'); ?></button>
                                <?php endif; ?>
                            </h2>
                        </div>
                        <div class="cardd mb-4" id="section-1">
                            ...
                            <div class="goals-area" id="top-goals-area" style="display:block; ">
                                <ol id="daily-top-goal-list" class="goal-list">
                                    ...
                                    <li class="<?= ($key > 9) ? 'hidden more' : ''; ?>"
                                        id="top-goal-list-item-<?= $item['id']; ?>" style="font-size: 1rem;">
                                        <label id="top-list-label-<?= $item['id']; ?>">

                                            <span style="font-size: 1rem;"
                                                id="topGoalText-<?= $item['id']; ?>"><?= $item['goal']; ?> </span>
                                            <input <?= ($isPastDate == true) ? 'disabled' : ''; ?>
                                                data-id="<?= $item['id']; ?>" value="<?= $item['id']; ?>"
                                                class="input-topgoals" name="topAchieved[<?= $item['id']; ?>]"
                                                type="checkbox" <?php if ($item['achieved'] == 1) echo 'checked'; ?>>
                                            <a class="edit-actions edit-goal-btn" data-type="top"
                                                data-id="<?= $item['id']; ?>" href="#"><i class="fa fa-pencil"></i></a>
                                            <a class="edit-actions delete-goal-btn" data-type="top"
                                                data-id="<?= $item['id']; ?>" href="#"><i class="fa fa-trash-o"
                                ...
                                <div class="form-group screenonly" style="padding:20px; text-align:right;"
                                    id="create-top-goal-btn-wrapper">

                                    <button type="button" id="save-new-top-goals-btn" style="display:none;"
                                        class="button btn btn-info" onClick="SaveNewTopGoals()"><i
                                            class="fa fa-save"></i> <?= translate('Guarda Nuevo Objetivo'); ?></button>

                                    <button type="button" class="button btn btn-info" onClick="CreateDailyTopGoal()"><i
                                            class="fa fa-book"></i> <?= translate('Agrega Objetivo'); ?></button>
                        ...
                        <div class="mt-5" style="background-color: #fef200; padding: 10px">
                            <h2 class="maintitle" style="padding:0; margin:0; width:100%; overflow:hidden; ">
                                <?= translate('La Acción o Resultado Más Importante Hoy'); ?>:
                                <?php if ($isPastDate == false) : ?>
                                <button type="button" class="btn btn-info btn-sm screenonly pull-right"
                                    id="impEditBtn"><?= translate('Editar'); ?></button>
                                <?php endif; ?>
                            </h2>
                        </div>
                        <div class="cardd mb-4" id="section-1">
                            ...
                            <div class="goals-area" id="important-goals-area" style="display:block; ">
                                <ol id="daily-important-goal-list" class="goal-list">
                                    ...
                                    <li class="<?= ($key > 9) ? 'hidden more' : ''; ?>"
                                        id="important-goal-list-item-<?= $item['id']; ?>" style="font-size: 1rem;">
                                        <label id="important-list-label-<?= $item['id']; ?>">

                                            <span style="font-size: 1rem;"
                                                id="importantGoalText-<?= $item['id']; ?>"><?= $item['goal']; ?> </span>
                                            <input <?= ($isPastDate == true) ? 'disabled' : ''; ?>
                                                data-id="<?= $item['id']; ?>" value="<?= $item['id']; ?>"
                                                class="input-importantgoals"
                                                name="importantAchieved[<?= $item['id']; ?>]" type="checkbox"
                                                <?php if ($item['achieved'] == 1) echo 'checked'; ?>>
                                            <a class="edit-actions edit-goal-btn" data-type="important"
                                                data-id="<?= $item['id']; ?>" href="#"><i class="fa fa-pencil"></i></a>
                                            <a class="edit-actions delete-goal-btn" data-type="important"
                                                data-id="<?= $item['id']; ?>" href="#"><i class="fa fa-trash-o"
                                                    aria-hidden="true"></i></a>
                                        </label>
                                    </li>
                                ...
                                <div class="form-group screenonly" style="padding:20px; text-align:right;"
                                    ...
                                    <button type="button" id="save-new-important-goals-btn" style="display:none;"
                                        class="button btn btn-info" onClick="SaveNewImpGoals()"><i
                                            class="fa fa-save"></i> <?= translate('Guarda Resultado'); ?></button>

                                    <button type="button" class="button btn btn-info"
                                        onClick="CreateDailyImportantGoal()"><i class="fa fa-book"></i>
                                        <?= translate('Resultado'); ?></button>
                        ...
                        <div class="cardd mb-5" id="section-2" style="padding:0 5px;">
                            <div class="d-flex justify-content-between my-1">
                                <h5 class="card-header" style="color:#FFF;  margin:5px 0; font-size: 1rem;">
                                    <?= translate('Mini Resumen de Hoy'); ?>:</h5>
                                <a href="<?= SITE_URL; ?>/users/evolutions.php"
                                    class="bg-primary py-1 px-2 rounded border border-primary text-white text-decoration-none"><?= translate('Más'); ?>
                                    <i class="fa fa-angle-double-right" aria-hidden="true"></i></a>
                            </div>
                            <div class="card-body">
                                <div class="form-group">
                                    <div class="description-area">
                                        <div class="print-description" id="print-evaluation"><?= $dailyEvolution; ?>
                                        </div>
                                        <textarea id="dailyEvolution" rows="5" class="LetterApplication editor ckeditor"
                                            name="dailyEvolution"><?= $dailyEvolution; ?></textarea>
                                    </div>
                                </div>
                            </div>
                        </div>
                        <div class="row px-1 mb-5">
                            <h5 class="" style="color:#FFF;  margin:5px 0; font-size: 1rem;">
                                <?= translate('Escribre 3 Etiquetas para facilitar la busqueda'); ?>
                                :</h5>
                            <?php for ($i = 0; $i < 3; $i++) : ?>
                            <div class="col-md-4 my-2">
                                <input type="text" class="form-control evaluation-tags" placeholder="Tag"
                                    name="evaluation_tag_1" value="<?= $evolutionTags[$i]['tag'] ?? '' ?>">
                            </div>
                            <?php endfor; ?>
                        </div>
                        ...
                        <div class="cardd my-5" id="section-2" style="padding:0 5px;">
                            <div class="d-flex justify-content-between my-1">
                                <h5 class="card-header" style="color:#FFF;  margin:5px 0; font-size: 1rem;">
                                    <?= translate('Mi Mayor Victoria Hoy'); ?>:</h5>
                                <a href="<?= SITE_URL; ?>/users/dailyVictories.php"
                                    class="bg-primary py-1 px-2 rounded border border-primary text-white text-decoration-none"><?= translate('Más'); ?>
                                    <i class="fa fa-angle-double-right" aria-hidden="true"></i></a>
                            </div>
                            <div class="card-body">
                                <div class="form-group">
                                    <div class="description-area">
                                        <div class="print-description" id="print-daily-victory">
                                            <?= $dailyVictory['daily_victory'] ?? ''; ?></div>
                                        <textarea id="daily_victory" class="LetterApplication editor ckeditor"
                                            name="daily_victory"><?= $dailyVictory['daily_victory'] ?? ''; ?></textarea>
                                    </div>
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="row px-1 mb-3">
                            ...
                            <?php for ($i = 0; $i < 3; $i++) : ?>
                            <div class="col-md-4 my-2">
                                <input type="text" class="form-control daily-victory-tags" placeholder="Tag"
                                    name="tag_1" value="<?= $dailyVictoryTags[$i]['tag'] ?? '' ?>">
                            </div>
                            <?php endfor; ?>
                        </div>
                        ...
                        <div class="cardd my-5" id="section-2" style="padding:0 5px;">
                            <div class="d-flex justify-content-between my-1">
                                ...
                                <a href="<?= SITE_URL; ?>/users/toRemember.php"
                                    class="bg-primary py-1 px-2 rounded border border-primary text-white text-decoration-none"><?= translate('Más'); ?>
                                    <i class="fa fa-angle-double-right" aria-hidden="true"></i></a>
                            ...
                            <div class="card-body">
                                <div class="form-group">
                                    <div class="description-area">
                                        <div class="print-description" id="print-to-remember">
                                            <?= $toRemember['to_remember'] ?? ''; ?></div>
                                        <textarea id="to_remember" class="LetterApplication"
                                            name="to_remember"><?= $toRemember['to_remember'] ?? ''; ?></textarea>
                                    </div>
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="cardd mb-5" id="section-3" style="padding:0 5px;">
                            ...
                            <div class="d-flex justify-content-between my-1">
                                <h5 class="card-header" style="color:#FFF; margin:5px 0; font-size: 1rem;">
                                    <?= translate('¿Cómo Puedo Mejorar?'); ?>: </h5>
                                <a href="<?= SITE_URL; ?>/users/improvements.php"
                                    class="bg-primary py-1 px-2 rounded border border-primary text-white text-decoration-none"><?= translate('Más'); ?>
                                    <i class="fa fa-angle-double-right" aria-hidden="true"></i></a>
                            ...
                            <div class="card-body">
                                <div class="form-group">
                                    <div class="description-area">
                                        <div class="print-description" id="print-improvements">
                                            <?= $dailyImprovements; ?></div>
                                        <textarea id="dailyImprovements" class="LetterApplication"
                                            name="dailyImprovements"><?= $dailyImprovements; ?></textarea>
                                    </div>
                                </div>
                            </div>
                        </div>

                        <div class="cardd mb-5" id="media-section"
                            style="padding:0 5px; margin-left:5px; margin-right:5px;">
                            <div class="card-body">
                                <div class="d-flex justify-content-end my-1">
                                    <a class="btn btn-sm button btn-info pull-right"
                                        href="<?= SITE_URL; ?>/users/victory-images.php?date=<?= $currentDate; ?>"><?= translate('Más'); ?><i
                                ...
                                <div class="d-flex flex-wrap bd-highlight mb-3" uk-lightbox="animation: slide">
                                    ...
                                    <div class="p-1 bd-highlight v7-media-box <?= $loopImage != null ? 'file-added' : ''; ?>"
                                        id="mediabox<?= $i + 1; ?>">
                                        <?php if ($loopImage != null) :  ?>
                                        <div class="media-thumb-wrapper" data-file="<?= $loopImage['id']; ?>"
                                            id="fileid-<?= $loopImage['id']; ?>">
                                            <a href="<?= $loopImage['url']; ?>" id="lightbox-thumb-item-<?= $i; ?>"
                                                data-index="<?= $i; ?>"> <img class="rounded-3"
                                                    src="<?= $loopImage['thumb']; ?>"></a>
                                            <div class="file-actions">
                                                <div class="dropdown">
                                                    <button class="btn btn-light btn-sm p-0" type="button"
                                                    ...
                                                    <ul class="dropdown-menu">
                                                        <li>
                                                            <div class="dropdown-item file_delete">
                                        ...
                                        <div class="upload-box-image upload-file-box" data-type="image">
                                            <input type="file" name="image<?= $i + 1; ?>File"
                                                id="image<?= $i + 1; ?>File" class="inputfile"
                                ...
                                <hr>
                                <div class="bd-highlight mb-3" id="AudioSection">
                                    <div class="d-flex justify-content-end my-1">
                                        <a class="btn btn-sm button btn-info pull-right"
                                            href="<?= SITE_URL; ?>/users/victory-media.php?type=audio&date=<?= $currentDate; ?>"><?= translate('Más'); ?><i
                                                class="fa fa-angle-double-right" aria-hidden="true"></i></a>
                                    </div>

                                    <div class="p-1 bd-highlight d-flex flex-wrap">
                                        <?php for ($i = 1; $i <= 5; $i++) : ?>
                                        <div class="p-1 v7-media-box <?= !empty($dailyV7Audios[$i - 1]) ? 'file-added has-audio-file' : ''; ?>"
                                            id="audioMediaBox" data-type="audio">
                                            <?php if (!empty($dailyV7Audios[$i - 1])) : ?>
                                            <!-- Existing audio file -->
                                            <div class="media-thumb-wrapper"
                                                data-file="<?= $dailyV7Audios[$i - 1]['id']; ?>"
                                                id="fileid-<?= $dailyV7Audios[$i - 1]['id']; ?>">
                                                <audio controls src="<?= $dailyV7Audios[$i - 1]['url']; ?>">
                                                    <a href="<?= $dailyV7Audios[$i - 1]['url']; ?>">
                                                ...
                                                <div class="file-actions">
                                                    <div class="dropdown">
                                                        ...
                                                        <ul class="dropdown-menu">
                                                            <li><a class="dropdown-item file_delete"
                                                                    href="#"><?= translate('Delete'); ?></a></li>
                                                        </ul>
                                                    </div>
                                                </div>
                                            </div>
                                            <?php else : ?>
                                            <!-- Upload option -->
                                            <div class="dropdown custom-audio-picker upload-box-image upload-file-box"
                                                data-type="audio">
                                                ...
                                                <ul class="dropdown-menu">
                                                    <li>
                                                        <input type="file" name="audio<?= $i; ?>File"
                                                            id="audio<?= $i; ?>File" class="inputfile dropdown-item"
                                                            accept="audio/mp3,audio/x-m4a,audio/*;capture=microphone" />
                                                        <label for="audio<?= $i; ?>File"><i class="fa fa-cloud-upload"
                                                                aria-hidden="true"></i> Upload</label>
                                                    </li>
                                                    <li><a class="dropdown-item" href="#" data-bs-toggle="modal"
                                                            data-bs-target="#audioRecorderModal"><i
                                                                class="fa fa-microphone"
                                                                aria-hidden="true"></i><?= translate('Record'); ?></a>
                                                    </li>
                                                </ul>
                                            </div>
                                            <?php endif; ?>
                                ...
                                <div class="d-flex bd-highlight mb-3">
                                    <div class="p-1 bd-highlight">
                                        ...
                                        <div class="v7-media-box <?= count($dailyV7Videos) > 0 ? 'file-added' : ''; ?>"
                                            id="videoMediabox4">
                                            <?php if (!empty($dailyV7Videos) && count($dailyV7Videos) > 0) :
                                                $poster = '';
                                                if (!empty($dailyV7Videos[0]['thumb'])) {
                                                    $poster = 'poster="' . $dailyV7Videos[0]['thumb'] . '"';
                                                } ?>
                                            <div class="media-thumb-wrapper" data-file="<?= $dailyV7Videos[0]['id']; ?>"
                                                id="fileid-<?= $dailyV7Videos[0]['id']; ?>">
                                                <video controls preload="metadata" <?= $poster; ?>>
                                                    <source src="<?= $dailyV7Videos[0]['url']; ?>#t=0.2"
                                                        type="video/mp4">
                                            ...
                                            <div class="upload-box-image upload-file-box" data-type="video">
                                                <input type="file" name="video1File" id="video1File" class="inputfile"
                                                    accept="video/*" />
                                    ...
                                    <div class="ms-auto p-1 bd-highlight">
                                        <a class="btn btn-sm button btn-info pull-right"
                                            href="<?= SITE_URL; ?>/users/victory-media.php?type=video&date=<?= $currentDate; ?>"><?= translate('Más'); ?><i
                                                class="fa fa-angle-double-right" aria-hidden="true"></i></a>
                        ...
                        <div class="py-2 py-3" style="background-color: #fef200; padding: 10px">
                            <h2 class="maintitle" style="padding:0; margin:0; width:100%; overflow:hidden;">
                                <?= translate('Tus 7-Objetivos y Prioridades Más Importantes para tu Vida'); ?>:
                                <?php if ($isPastDate == false) : ?>
                                <button type="button" class="btn btn-sm btn-info screenonly pull-right"
                                    id="editBtn2"><?= translate('Editar'); ?></button>
                                <?php endif; ?>
                            </h2>
                        </div>
                        <div class="cardd" id="section-4">
                            ...
                            <div class="goals-area" id="life-goals-area" style="display:block;">
                                <ol style="font-size: 1rem;" id="daily-life-goal-list" class="goal-list">
                                    ...
                                    <li class="<?= ($key > 9) ? 'hidden more' : ''; ?>"
                                        id="life-goal-list-item-<?= $item['id']; ?>">
                                        <label id="life-list-label-<?= $item['id']; ?>">

                                            <span class="lifeGoalText"
                                                id="lifeGoalText-<?= $item['id']; ?>"><?= nl2br($item['goal']); ?>
                                            </span>
                                            <input <?= ($isPastDate == true) ? 'disabled' : ''; ?>
                                                data-id="<?= $item['id']; ?>" value="<?= $item['id']; ?>"
                                                class="input-lifegoals" name="lifeAchieved[<?= $item['id']; ?>]"
                                                type="checkbox" <?php if ($item['achieved'] == 1) echo 'checked'; ?>>
                                            <a class="edit-actions edit-goal-btn" data-type="life"
                                                data-id="<?= $item['id']; ?>" href="#"><i class="fa fa-pencil"></i></a>
                                            <a class="edit-actions delete-goal-btn" data-type="life"
                                                data-id="<?= $item['id']; ?>" href="#"><i class="fa fa-trash-o"
                                                    aria-hidden="true"></i></a>
                                        </label>
                                    </li>
                                ...
                                <?php if ($isPastDate == false) : ?>
                                <div class="form-group screenonly" style="padding:20px; text-align:right;"
                                    id="create-life-goal-btn-wrapper">
                                    <button type="button" id="save-new-life-goals-btn" style="display:none;"
                                        class="button btn btn-info" onClick="SaveNewLifeGoals()"><i
                                            class="fa fa-save"></i> <?= translate('Guarda Nuevo Objetivo'); ?></button>
                                    <button type="button" class="button btn btn-info" onClick="CreateDailyLifeGoal()"><i
                                            class="fa fa-book"></i> <?= translate('Agrega Objetivo'); ?></button>
                                </div>
                                <?php endif; ?>
                            </div>
                        ...
                        <div id="section_box_wrapper" class="section_box_wrapper">
                            ...
                            <div class="section_box" id="section_box_<?= $bitem['id'] ?>">
                                ...
                                <div class="section_content"><textarea name="box[<?= $bitem['id'] ?>]"
                                        data-box="<?= $bitem['id'] ?>" id="boxitem-<?= $bitem['id'] ?>"
                                        class="LetterApplication boxitem"><?= $bitem['body'] ?></textarea></div>
                            </div>
                            <?php endforeach; ?>
                        ...
                        <div class="load-btn-wrapper mt-5 mb-5 text-center">
                            <button class="btn btn-lg btn-warning"
                                id="btnLoadMoreTenSections"><?= translate('Mostrar más'); ?></button>
                        </div>

                        <div style="display: none;" id="show">
                            <div style="padding: 15px; border-radius: 7px; margin-bottom: 15px;display: flex; align-content: center; justify-content: space-between;align-items: center;"
                                id="error_success_msg_verification" class="msg">
                                <p id="success_msg_verification_text" style="font-size: 14px; font-weight: 600;"></p>
                                <button
                                    style="border: 0px; background: transparent; font-size: 18px; font-weight: 800;align-items: center;"
                                    id="close">x</button>
                            </div>
                        </div>
                        <div class="form-group screenonly">
                            <div class="button-wrapper" style="margin:30px 0;">
                                <button class="btn btn-info letter" type="button" data-bs-toggle="modal"
                                    href="#exampleModalToggle" role="button"><?= translate('Email'); ?></button>

                                <input class="btn btn-info letter" type="button" id="savePrintBtn" name="savePrintBtn"
                                    value="<?= translate('Guardar pdf'); ?>" />

                                <input class="btn btn-info letter" type="button" id="saveBtn" name="saveBtn"
                ...
                <div class="pt-5" id="slide-2">
                    <div class="cardd mb-4">
                        <div class="mt-5 mb-3" style="background-color: #fef200; padding: 10px">
                            <h2 class="maintitle text-black " style="padding:0; margin:0; width:100%; overflow:hidden;">
                                <?= translate('Citas y Eventos'); ?>:</h2>
                        </div>
                        <?php include_once 'inc/appointments.php'; ?>
                        <div class="mt-5 mb-3" style="background-color: #fef200; padding: 10px">
                            <h2 class="maintitle text-black " style="padding:0; margin:0; width:100%; overflow:hidden;">
                                <?= translate('Para Hacer Hoy'); ?>:</h2>
                        </div>
                        <?php include_once 'inc/toBeDone.php'; ?>
                        <div class="mt-5 mb-3" style="background-color: #fef200; padding: 10px">
                            <h2 class="maintitle text-black " style="padding:0; margin:0; width:100%; overflow:hidden;">
                                <?= translate('Ingresos y gastos'); ?>:</h2>
                        </div>
                ...
                <!-- Silder Navigation -->
                <button class="bg-primary py-1 px-3 rounded border border-primary text-white" id="prev-btn"><i
                        class="fa fa-angle-double-left" aria-hidden="true"></i></button>
                <button class="bg-primary py-1 px-2 rounded border border-primary text-white"
                    id="next-btn"><?= translate('Más'); ?> <i class="fa fa-angle-double-right"
        ...
        <div class="modal-content">
            ...
            <div class="modal-body">
                <div class="form-group">
                    ...
                    <input style="width:100%;" type="email" class="form-control" name="toemail" id="toEmail"
            ...
            <div class="modal-footer">
                ...
                <button class="btn btn-primary" type="button" id="sendBtn"
                    name="sendBtn"><?= translate('Send Email'); ?></button>
            </div>
        </div>
    </div>
</div>
...
<div class="modal fade p-0" id="audioRecorderModal" tabindex="-1" aria-labelledby="audioRecorderModalLabel"
    aria-hidden="true">
                    ...
                    <div class="audio-recording-container text-white text-center" id="audio-recorder">
                        <div class="recording-info-wrapper" id="recording-info">
                            <div class="recording-elapsed-time">
                                ...
                                <p class="max-duration-label"><?= translate('Max duration 15 minutes'); ?></p>
                            </div>
                            <div id="recordingsList"></div>
                            <div class="controls">
                                <button disabled
                                    class="rounded-circle border border-3 btn btn-danger stop-recording-button "
                                    id="stopButton"> <i class="fa fa-circle" aria-hidden="true"></i></button>
                                <button disabled
                                    class="rounded-circle border border-3 btn btn-danger cancel-recording-button "
                                    id="pauseButton"><i class=" fa fa-pause" aria-hidden="true"></i></button>
                            </div>
                        </div>
                        <button style="margin:2rem auto;"
                            class="rounded-circle border border-3 btn btn-danger start-recording-button"
                            id="recordButton"><i class=" fa fa-microphone" aria-hidden="true"></i></button>
            ...
            success: function(data) {
                ...
                if (jsonObj.success) {
                    ...
                    for (const prop in jsonObj.goals) {
                        ...
                        $("#daily-life-goal-list").append(
                            '<li style="font-size: 1rem;" id="life-goal-list-item-' + prop +
                            '"><label class="form-label" id="life-list-label-' + prop +
                            '"><span class="lifeGoalText" id="lifeGoalText-' + prop + '">' + jsonObj
                            .goals[prop] + '</span> <input name="lifeAchieved[' + prop +
                            ']" class="input-lifegoals" type="checkbox" data-id="' + prop +
                            '" value="' + prop +
                            '"><a class="edit-actions edit-goal-btn" data-type="life" data-id="' +
                            prop +
                            '" href="#"><i class="fa fa-pencil"></i></a>                 <a class="edit-actions delete-goal-btn" data-type="life" data-id="' +
                            prop +
                            '" href="#"><i class="fa fa-trash-o" aria-hidden="true"></i></a></label></li>'
                        );
                    }
            ...
            success: function(data) {
                ...
                if (jsonObj.success) {
                    ...
                    for (const prop in jsonObj.goals) {
                        ...
                        $("#daily-important-goal-list").append(
                            '<li class="" id="important-goal-list-item-' + prop +
                            '"><label class="form-label" id="important-list-label-' + prop +
                            '"><span id="importantGoalText-' + prop + '">' + jsonObj.goals[prop] +
                            '</span> <input name="achieved[' + prop +
                            ']" class="input-importantgoals" type="checkbox" data-id="' + prop +
                            '" value="' + prop +
                            '"><a class="edit-actions edit-goal-btn" data-type="important" data-id="' +
                            prop +
                            '" href="#"><i class="fa fa-pencil"></i></a>                 <a class="edit-actions delete-goal-btn" data-id="' +
                            prop +
                            '" href="#"><i class="fa fa-trash-o" aria-hidden="true"></i></a></label></li>'
                        );
                    }
            ...
            success: function(data) {
                ...
                if (jsonObj.success) {
                    ...
                    for (const prop in jsonObj.goals) {
                        ...
                        $("#daily-top-goal-list").append('<li class="" id="top-goal-list-item-' + prop +
                            '"><label class="form-label" id="top-list-label-' + prop +
                            '"><span id="topGoalText-' + prop + '">' + jsonObj.goals[prop] +
                            '</span> <input name="achieved[' + prop +
                            ']" class="input-topgoals" type="checkbox" data-id="' + prop + '" value="' +
                            prop + '"><a class="edit-actions edit-goal-btn" data-type="top" data-id="' +
                            prop +
                            '" href="#"><i class="fa fa-pencil"></i></a>                 <a class="edit-actions delete-goal-btn" data-id="' +
                            prop +
                            '" href="#"><i class="fa fa-trash-o" aria-hidden="true"></i></a></label></li>'
...
function CreateDailyImportantGoal() {
    ...
    if (validated && remainingImportantGoals > 0) {
        $wrapper.append(
            "<div class='form-group'><textarea placeholder='Write goal details' class='form-input form-control newimpgoals' name='newimpgoals[]'/></textarea></div>"
        );
...
function CreateDailyTopGoal() {
    ...
    if (validated && remainingTopGoals > 0) {
        $wrapper.append(
            "<div class='form-group'><textarea placeholder='Write goal details' class='form-input form-control newtopgoals' name='newtopgoals[]'/></textarea></div>"
        );
        remainingTopGoals--;
...
function addTextArea(wrapperId, itemClass, name, placeholder, classes) {
    $wrapper = $(`#${wrapperId}`);
    $wrapper.append(
        `<li class="${itemClass}"><div class="form-group"><textarea class="w-100 text-white mt-2 bg-transparent text-bottom-border ${classes}" name=${name}/></textarea></div></li>`
    );
}
...
function CreateDailyLifeGoal() {
    ...
    if (validated && remainingLifeGoals > 0) {
        $wrapper.append(
            "<div class='form-group'><textarea placeholder='Write goal details' class='form-input form-control newlifegoals' name='newlifegoals[]'/></textarea></div>"
        );
        remainingLifeGoals--;
...
function CreateGoal(type) {
    ...
    if (validated) {
        $wrapper.append(
            "<div class='form-group'><textarea placeholder='Write goal details' class='form-input form-control newgoals' name='newgoals[]'/></textarea></div>"
        );
        goalstobeadded++;
        if (goalstobeadded > 0) {
            $('#save-new-top-goals-btn').show();
...
$(document).on('click', '.file-actions .file_delete', function(e) {
    ...
    $.ajax({
        ...
        success: function(data) {
            ...
            $('#AudioSection').load(window.location.href + ' #AudioSection');
        }
    });
...
$(document).on('click', '.edit-goal-btn', function(e) {
    ...
    if ($(this).find('.fa').hasClass('fa-pencil')) {
        ...
        $("#" + containterItemId).append('<textarea id="edittextarea-' + sectionType + goalId + '">' +
            goalText + '</textarea>');
    } else {
...
function UpdateData() {
    ...
    document.querySelectorAll('.daily-victory-tags').forEach(tag => tag.value.trim() !== '' ? dailyVictoryTags.push(tag
        .value) : '');
    const evaluationTags = [];
    document.querySelectorAll('.evaluation-tags').forEach(tag => tag.value.trim() !== '' ? evaluationTags.push(tag
        .value) : '');
...
function createFilePreviewEle(id, url, type, $wrapper) {
    ...
    if (type == 'video/mp4' || type.startsWith("video")) {
        //filePreview= `<img alt="preview" class="files_img rounded-3" src="${url}"/>`;
        filePreview =
            `<video id="video-preview-player" alt="preview" class="files_img rounded-3" src="${url}"/></video>`;
...
function handleFileUpload(files, upload_type, $fileWrapperElem, thumbURI = '') {
    console.log('Upload Type (handleFileUpload):', upload_type);
    let addFiles = [];
    for (let i = 0; i < files.length; i++) {
        ...
        if (!upload_type) {
            upload_type = 'audio';
        } else if (upload_type === 'audio' && type === '') {
            type = 'audio/wav';
        }
            ...
            $.ajax({
                ...
                success: function(json) {
                    ...
                    if (response.success) {
                        ...
                        if (response.type == 'audio') {
                            ...
                            $fileWrapperElem.find('.media-thumb-wrapper').prepend(
                                $audioElm);
                            $fileWrapperElem.addClass("has-audio-file");
                            $('#AudioSection').load(window.location.href +
                                ' #AudioSection');
                  ...
                  <source src="${response.url}#t=0.2" type="video/mp4">
                        Your browser does not support the video tag.</video>`);
                            ...
                            let $audioElm = $(
                                `<a href="${response.file_url}" > <img class="rounded-3"  src="${response.url}"></a>`
                            );
                            $fileWrapperElem.find('.media-thumb-wrapper').prepend(
                                $audioElm);
                        }
                ...
                xhr: function() {
                    ...
                    xhr.upload.addEventListener('progress', function(e) {
                        ...
                        $fileWrapperElem.find('.progress-mask').innerHTML = Math
                            .ceil(progressRate) + '%';
            ...
            media.addEventListener('loadedmetadata', () => {
                ...
                if (duration > maxAllowed) {
                    ...
                    if (upload_type == 'audio') {
                        showToast('error',
                            "Only maximum 15 minutes of audio is allowed. This audio has duration " +
                            getDurationTIme(duration));
                    } else if (upload_type == 'video') {
                        showToast('error',
                            "Only maximum 1 minute of video is allowed. This video has duration " +
                            getDurationTIme(duration));
                    }
...
UIkit.util.on(document, 'itemshown', '.uk-lightbox.uk-open', function(event, lightbox) {
    ...
    mc.on('pinch', function(e) {
        ...
        zoomLevel = Math.max(1, Math.min(3, zoomLevel * e
            .scale)); // Adjust the maximum and minimum zoom levels as needed
...
<div id="popovertip2" class="popovertip" data-days="3" data-page="dailygoalsbackup"
    data-bs-custom-class="mejor-info-popover white-popover bs-popover-bottom 1st popovertip2" data-bs-toggle="popover"
    data-bs-placement="bottom"
    data-bs-content="<p><?= translate('Te Gustaría hacer un Backup para no Perder tu Info?'); ?> </p><a class='btn btn-warning mt-2' href='<?= SITE_URL; ?>/users/backup.php'><?= translate('Haz Click Aquí'); ?></a>">
</div>
<div id="popovertip" class="popovertip" data-page="dailygoals"
    data-bs-custom-class="mejor-info-popover bs-popover-bottom 2nd" data-bs-toggle="popover" data-bs-placement="bottom"
    data-bs-content="<?= translate('popover_dailygoals_1'); ?>"></div>
